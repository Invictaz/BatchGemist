...
FOR /F "delims=" %%A IN ('ECHO %s_json% ^| %xidel% - -e "count($json())"') DO (
	IF "%%A"=="1" (
		...
		ECHO.
		ECHO %s_json% | %xidel% - ^
		--xquery ^"let $width:^=string-length^(^
		            count^($json^(^)^)^
		          ^)^
		          for $x at $i in $json^(^) return^
		          '  '^|^|join^(^
...
FOR /F "delims=" %%A IN ('ECHO %s_json% ^| %xidel% -
-e ^"p_json:^=x:request^(
      {
        'data':concat^(
...
IF /I "%remap%"=="j" (
	...
	-e ^"let $a:^=read^(^) return
	    if ^($a^) then
	      ...
	      '%~dp0'^" --output-encoding^=oem^"') DO SET "map=%%A"
) ELSE (
	SET "map=%~dp0"
)
...
IF /I "%rename%"=="j" (
	...
	-e ^"let $a:^=read^(^) return
	    if ^($a^) then
	      ...
	      '%name%'^" --output-encoding^=oem^"') DO SET "name=%%A"
)
